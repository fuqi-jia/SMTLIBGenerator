...
std::shared_ptr<SMTLIBParser::DAGNode> Generator::generateArithmeticExpression(int depth, const std::shared_ptr<SMTLIBParser::Sort>& sort) {
    ...
    if (param_count == 1) {
        ...
        // | $\text{acosh}\,x$  | $[1, +\infty)$                    |
            ...
            if (child_value < 1) {
                ...
                if(child_value < -1){
                    ...
                    auto epsilon = generateConstant(sort);
                    if(parser->toReal(epsilon) <= 0){
                        epsilon = parser->mkNeg(epsilon);
                    }
        ...
        if ((selected_op == SMTLIBParser::NODE_KIND::NT_DIV_INT ||
            ...
            // log(x, base)
            auto left_value = parser->toReal(parser->evaluate(left, model));
            if(left_value <= 0){
                // 如果底数为负数，替换为非负数
                left = parser->mkAbs(left);
                left = parser->mkAdd(left, parser->mkConstReal(1));
            }

            auto right_value_ = parser->toReal(parser->evaluate(right, model));
            if(right_value_ < 0){
                // 如果指数为负数，替换为非负数
                right = parser->mkAbs(right);
                right = parser->mkAdd(right, parser->mkConstReal(1));
            }
            else if(right_value_ == 0){
                // 如果指数为0，替换为非0值
                right = parser->mkAdd(right, parser->mkConstReal(2));
            }
            else if(right_value_ == 1.0){
                // 如果指数为1，替换为非1值
                right = parser->mkAdd(right, parser->mkConstReal(1));
            }
        }
        // std::cout<<dumpSMTLIB2(left)<<std::endl;
        // std::cout<<dumpSMTLIB2(right)<<std::endl;
...
void Generator::generateSMTLIB2File(const std::string& output_path, int num_vars, int num_constraints) {
    ...
    for (const auto& constraint : constraints) {
        outfile << SMTLIBParser::dumpSMTLIB2(constraint) << std::endl;
    }

    outfile.close();
    std::cout << "Successfully generated constraint file: " << output_path << std::endl;
    ...
    if (output_file_path.parent_path().empty()) {
        // 如果输出文件在当前目录，则模型文件也生成在当前目录
        model_path = output_file_path.stem().string() + ".json";
    } else {
        // 否则生成在与输出文件相同的目录
        model_path = output_file_path.parent_path().string() + "/" +
                     output_file_path.stem().string() + ".json";
    }

    std::ofstream model_outfile(model_path);
    if (!model_outfile.is_open()) {
        std::cerr << "Error: Unable to open model file: " << model_path << std::endl;
        return;
    }

    model_outfile << "{" << std::endl;
    model_outfile << "  \"solutions\": [" << std::endl;
    model_outfile << "    {" << std::endl;
